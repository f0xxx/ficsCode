    ...
    : public ZKEventSource
{
    public:
        ...
        bool createSequenceEx(const string &path,  
							   string &createdPath,                      
							   const char *value = "", 
							   int valueLength = 0,
                               int flags = 0, 
...
//  	bool nodeExistsEx(const string &path,
//                           watcher_fn listener,
...
//  	throw(ZooKeeperException);
        ...
        void getNodeChildrenEx(String_vector *children,
                             const string &path, 
                             ZKEventListener *listener = NULL, 
                             void *context = NULL) 
            throw(ZooKeeperException);               
         ...
         * @throw ZooKeeperException if the operation has failed
         */
        void getNodeData(const string &path, 
						   char *value,
						   int &valueLength,
						   ZKEventListener *listener = NULL, 
                           void *context = NULL,
         ...
         * @throw ZooKeeperException if the operation has failed
         */
        void setNodeData(const string &path, const char *value,int valueLength, int version = -1) 
            throw(ZooKeeperException);
		...
		void setReconnectStatus(bool par);
    ...
    private:
		...
		void LogContext(const Listener2Context &context);
        /**
