...
#define ADAPTER_LOG_TRACE(logger, fmt, args...) \
    if (0) { \
        SPRINTF_LOG_MSG( __tmp, fmt, ##args ); \
        STRACE("%s",__tmp); \
    }

#define ADAPTER_LOG_DEBUG(logger, fmt, args...) \
    if (0) { \
        SPRINTF_LOG_MSG( __tmp, fmt, ##args ); \
        STRACE("%s",__tmp); \
    }

#define ADAPTER_LOG_INFO(logger, fmt, args...) \
    if (0) { \
        SPRINTF_LOG_MSG( __tmp, fmt, ##args ); \
        STRACE("%s",__tmp); \
    }

#define ADAPTER_LOG_WARN(logger, fmt, args...) \
    if (1) { \
        SPRINTF_LOG_MSG( __tmp, fmt, ##args ); \
        VTRACE("%s",__tmp); \
    }

#define ADAPTER_LOG_ERROR(logger, fmt, args...) \
    if (1) { \
        SPRINTF_LOG_MSG( __tmp, fmt, ##args ); \
        VTRACE("%s",__tmp); \
    }

#define ADAPTER_LOG_FATAL(logger, fmt, args...) \
    if (1) { \
        SPRINTF_LOG_MSG( __tmp, fmt, ##args ); \
        VTRACE("%s",__tmp); \
    }
...
#ifdef DISABLE_TRACE
#   define ADAPTER_TRACE(logger, x)
#else   
#   define ADAPTER_TRACE(logger, x) \
class AdapterTrace { \
 public: \
    AdapterTrace(const void* p) : _p(p) { \
        ADAPTER_LOG_TRACE(logger, "%s %p Enter", __PRETTY_FUNCTION__, p); \
    } \
    ~AdapterTrace() { \
        ADAPTER_LOG_TRACE(logger, "%s %p Exit", __PRETTY_FUNCTION__, _p); \
    } \
    const void* _p; \
} adapterTraceObj(x);
#endif  /* DISABLE_TRACE */
