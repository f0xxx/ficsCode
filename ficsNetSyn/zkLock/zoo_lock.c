...
//          ret = zoo_delete(zh, buf, -1);
            ret = zkDataBase::GetZkDataBase().DeleteNode(delPath).GetSysErr();
...
static void lock_watcher_fn(void *watcherCtx)
{
    //callback that we registered 
    //should be called
    zkr_lock_mutex_t *mutex = (zkr_lock_mutex_t *)watcherCtx;
    NETSYN_LOG_ERR("===lock_watcher_fn=== call setEvt");
    mutex->evt->Set();
...
//      ret = zoo_get_children(zh, path, 0, vector);
        ret = zkDataBase::GetZkDataBase().GetNodeChildrenEx(vector,getPath).GetSysErr();
        if (ret == ZCONNECTIONLOSS) {
...
static int retry_zoowexists(zhandle_t *zh, char* path, ZKDATABASEFUC watcher, void* ctx,
                            struct Stat *stat, struct timespec *ts, int retry) {
    int ret = ZCONNECTIONLOSS;
    int count = 0;
...
//      ret = zoo_wexists(zh, path, watcher, ctx, stat);
        ret = zkDataBase::GetZkDataBase().NodeExists(nodePath,watcher,ctx,stat).GetSysErr();
        if (ret == ZCONNECTIONLOSS) {
...
static int zkr_lock_operation(zkr_lock_mutex_t *mutex, struct timespec *ts) {
    ...
    int retry = 1;
    do {
        const clientid_t *cid = zoo_client_id(zkDataBase::GetZkDataBase().GetZkHandle());
        // get the session id
...
//          char retbuf[len+20];
            ...
            ret = ZCONNECTIONLOSS;
//          ret = zoo_create(zh, buf, NULL, 0,  mutex->acl,ZOO_EPHEMERAL|ZOO_SEQUENCE, retbuf, (len+20));
            string createPath(buf);
            string retPath;
            ret = zkDataBase::GetZkDataBase().  \
                CreateSequenceNodeEx(createPath,    \
                                     retPath,       \
                                     ZOO_EPHEMERAL, \
                                     zkDataBase::GetZkDataBase().GetLocNodeIPStr().c_str(), \
                                     zkDataBase::GetZkDataBase().GetLocNodeIPStr().length()).GetSysErr();
            ...
            if (mutex->id) 
                free(mutex->id);
            mutex->id = getName((char *)retPath.c_str());
        }
        ...
        if (mutex->id != NULL) {
            ...
            if (lessthanme != NULL) {
                ...
                if (ret != ZOK) {
                    free_String_vector(vector);
                    LOG_WARN(("unable to watch my predecessor,ret[%d]",ret));
...
//  int exists = zoo_exists(zh, path, 0, &stat);
    int exists = zkDataBase::GetZkDataBase().NodeExists(nodePath,NULL,NULL,&stat).GetSysErr();
    int count = 0;
    struct timespec ts;
    ts.tv_sec = 0;
    ts.tv_nsec = (.5)*1000000;
...
//          exists = zoo_exists(zh, path, 0, &stat);
            exists = zkDataBase::GetZkDataBase().NodeExists(nodePath,NULL,NULL,&stat).GetSysErr();
        else if (exists == ZNONODE) 
//          exists = zoo_create(zh, path, NULL, 0, mutex->acl, 0, NULL, 0);
            exists = zkDataBase::GetZkDataBase().CreateNode(nodePath).GetSysErr();
        nanosleep(&ts, 0);
